    ...
    return (
      <div className="bg-deep-earth text-white relative">
        <div className="container mx-auto px-4 py-2.5 text-center text-sm font-inter">
          <p>
            🎉 <strong>{t('discount')}</strong> {t('firstOrder')} {t('withCode')}: <strong>{t('welcomeCode')}</strong>
          </p>
        </div>
        <Button
          variant="ghost"
      ...
      case 'UK':
        return (
          <>
            ...
            <strong>{formatCurrency(ukShipping.freeDeliveryThreshold, 'GBP')}</strong> |{' '}
            <strong>{t('nextDayAvailable')}</strong> |{' '}
            <strong>{t('discount')}</strong> {t('firstOrder')}: <strong>{t('welcomeCode')}</strong>
          </>
        );
      case 'US':
        return (
          <>
            🎉 <strong>{t('weShipTo', { zone: 'the USA' })}</strong> {t('getProducts')} |{' '}
            <strong>{t('discount')}</strong> {t('firstOrder')}: <strong>{t('welcomeCode')}</strong>
          </>
        );
      case 'UAE':
        return (
          <>
            🎉 <strong>{t('weShipTo', { zone: 'the UAE' })}</strong> {t('getProducts')} |{' '}
            <strong>{t('discount')}</strong> {t('firstOrder')}: <strong>{t('welcomeCode')}</strong>
          </>
        );
      case 'EU':
